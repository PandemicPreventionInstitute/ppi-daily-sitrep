...
if (USE_CASE == 'databricks'){
    write.csv(omicron_gisaid, '/dbfs/FileStore/tables/omicron_gisaid_feed.csv', header = TRUE, row.names = FALSE)
    write_csv(gisaid_metadata, '/dbfs/FileStore/tables/metadata.csv', header = TRUE, row.names = FALSE)
}
