...
# Global dashboard update for BA.4/5
...
gisaid_t <- gisaid_raw%>%select(collection_date, gisaid_country, n_new_sequences, ba_4_5, 
...
gisaid_t <- gisaid_t %>%
  ...
  mutate(
    ...
    rolling_ba_4_5_last_7_days = rollapplyr(ba_4_5, 7, sum, partial = TRUE, align = "right"),
    rolling_seq_last_7_days = rollapplyr(n_new_sequences, 7, sum, partial = TRUE, align = "right"),
    rolling_pct_of_seq_BA4_5 = round(100*rolling_ba_4_5_last_7_days/rolling_seq_last_7_days,2)
  )
...
cases_in_last_7_days<-gisaid_t%>%filter(collection_date>=(LAST_DATA_PULL_DATE - TIME_WINDOW_WEEK) & 
  ...
  summarise(cases_per_100k_last_7_days = round(100000*sum(owid_new_cases)/max(owid_population, na.rm = TRUE), 1),
            pct_cases_BA45_past_7_days = round(100*sum(ba_4_5, na.rm = T)/sum(n_new_sequences, na.rm = T), 2))

# join with 30 day summary 
gisaid_recent_data<-left_join(gisaid_recent_data, cases_in_last_7_days, by = "country_code")


# --- Find number of BA.4/BA.5 cases submitted by country ---- 
gisaid_BA4_5<-gisaid_t%>%
  group_by(country_code)%>%
  summarise(n_ba_4_5 = sum(ba_4_5, na.rm = TRUE),
            n_sequences = sum(n_new_sequences, na.rm = TRUE))

gisaid_recent_data<-left_join(gisaid_recent_data, gisaid_BA4_5, by = "country_code")
...
gisaid_recent_data<-gisaid_recent_data%>%
  mutate(tests_per_100k_in_last_7_days= 100000*tests_in_last_7_days/population_size,
        ...
        pct_cases_BA4_5_past_30_days = round(100*n_ba_4_5/n_sequences,2))

gisaid_BA4_5_past_week<-gisaid_t%>%filter(collection_date == (LAST_DATA_PULL_DATE - 7))%>%group_by(country_code)%>%
  summarise(pct_seq_BA_4_5_last_week = max(rolling_pct_of_seq_BA4_5),
...
gisaid_recent_data<-left_join(gisaid_recent_data, gisaid_BA4_5_past_week, by = "country_code")
...
gisaid_summary_df$n_ba_4_5[is.na(gisaid_summary_df$n_ba_4_5)]<-0 # set countries without ba_2 as 0 
gisaid_summary_df$n_ba_4_5_NA<-gisaid_summary_df$n_ba_4_5 # copy over col with 0s
gisaid_summary_df$n_ba_4_5_NA[gisaid_summary_df$n_ba_4_5 ==0]<-NA # replace 0s with NA
gisaid_summary_df<-gisaid_summary_df%>%
  mutate(ba_4_5_detected = case_when(
    n_ba_4_5 >0 ~ 'Yes',
    n_ba_4_5 == 0 ~'No',
    is.na(n_ba_4_5) ~'No'
  ))
...
gisaid_summary_df$pct_BA_4_5_dots<-gisaid_summary_df$pct_seq_BA_4_5_last_week
gisaid_summary_df$pct_BA_4_5_dots[gisaid_summary_df$ba_4_5_detected == "No"]<-NA
gisaid_summary_df$pct_BA_4_5_dots[(gisaid_summary_df$pct_seq_BA_4_5_last_week == 0 | is.na(gisaid_summary_df$pct_seq_BA_4_5_last_week)) & gisaid_summary_df$ba_4_5_detected == "Yes"]<-0

# Make a column for the pop-up
gisaid_summary_df$pct_BA4_5_panel<-gisaid_summary_df$pct_seq_BA_4_5_last_week
gisaid_summary_df$pct_BA4_5_panel[gisaid_summary_df$total_seq_last_week<100]<-paste0(gisaid_summary_df$pct_seq_BA_4_5_last_week[gisaid_summary_df$total_seq_last_week<100], ' %*')
gisaid_summary_df$pct_BA4_5_panel[gisaid_summary_df$total_seq_last_week>100]<-paste0(gisaid_summary_df$pct_seq_BA_4_5_last_week[gisaid_summary_df$total_seq_last_week>100], ' %')
gisaid_summary_df$pct_BA4_5_panel[gisaid_summary_df$total_seq_last_week==0]<-'No sequences were submitted from last week'
...
stopifnot('USA BA4_5 data not being detected' = US_df$n_ba_4_5>40)
stopifnot('Country reporting greater than 100% BA.2' = sum(gisaid_summary_df$pct_BA_4_5_dots>100, na.rm = T)==0)
...
stopifnot ("Error: gisaid_summary_df.csv does not contain all necessary columns" = 
             c('geometry', 'latitude', 'longitude', 'Name', 'n_ba_4_5',
               'n_ba_4_5_NA', 'ba_4_5_detected', 'cases_per_100k_last_7_days', 'pct_BA_4_5_dots', 'pct_BA4_5_panel') %in% colnames(gisaid_summary_df))
...
gisaid_summary_df<- gisaid_summary_df %>% select(geometry,latitude, longitude, Name, n_ba_4_5, 
                                                 n_ba_4_5_NA, ba_4_5_detected, cases_per_100k_last_7_days, 
                                                 percent_of_cases_sequenced_last_30_days, rounded_pct_cases_seq,
                                                 pct_cases_seq_w_NA, sequences_in_last_30_days, pct_seq_BA_4_5_last_week, pct_BA_4_5_dots, pct_BA4_5_panel, total_seq_last_week)
...
if (USE_CASE == "domino"){
  write.csv(gisaid_summary_df, "/mnt/data/processed/gisaid_summary_df_ba4_5.csv")
}

if (USE_CASE == "local"){
  write.csv(gisaid_summary_df, "../data/processed/gisaid_summary_df_ba4_5.csv")
}
